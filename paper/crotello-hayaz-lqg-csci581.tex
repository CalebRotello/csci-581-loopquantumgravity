...
%
%We had access to Google's Weber QPU 
...
%a description of gravity according to quantum mechanics. 
%There are many proposed quantum gravity theories, but all of 
%them still need to overcome major problems. Besides these problems currently, there is no way 
%to put gravity prediction in the experimental test environment. Currently, there are two main 
%theories to quantum gravity theories are String theory and Loop quantum gravity (LQG).

%% Hakan
%Loop Quantum Gravity (LQG), is a theory based on the quantization of space-time, 
%where entangled quantum tetrahedra give rise to space-time in a way the unifies quantum mechanics and general relativity.
%Because of the complexity in the Planck scale, it is an extremely difficult task for classical computers however, 
%current quantum computing technology gives an exponential speedup to problems and a dramatic increase in spatial efficiency that supercomputers cannot
%do. 
%Because of the storage capabilities of the Bloch sphere, a quantum computer with 40 qubits can easily 
%manipulate a system that has $2^{40}$ states.


\section{Loop Quantum Gravity}
%Stronger introduction to loop quantum gravity
%Loop quantum gravity is one of the most exciting problems in quantum gravity research. Because LQG helps us to 
%understand time, space, and matter. Besides understanding time, space, and matter LQG allows us to explore the domain and 
%range of early universe and black hole thermodynamic \cite{covariant-lqg}.
        ...
        initial 3-dimensional space states to final 3-dimensional space states creates the 4-dimensional quantum spacetime regions \cite{covariant-lqg}. %\cite{ibm-qsim-qubit-of-space}.
        Spin-foam amplitudes are the probability amplitudes of the spin-foam model after one such evolution.


%A covariant formulation of LQG is related to the Spin Foam model (SFM) and for example, 
%3-dimensional space is described by a quantum state with a 3-dimensional space state. In the spin-foam model, 
%related question is answered by the spin foam amplitudes which are probability amplitudes of 4-dimensional quantum space-time 
%results. %That’s why calculations begin with a quantum tetrahedron which is the simplest discrete unit of 3D space.
...
\subsection{Quantum Tetrahedra and Spin-Networks}
        ...
        of LQG is quantized space-time, and the tetrahedron is the simplest 3 dimensional geometry that can be used for this quantization. %\textbf{CITATION - WHY tetrahedra?}. 
...
%The transition amplitdue of a quantum gravitational system, or spin-foam amplitude, is a probability amplitude of a 
%The theoretical process at work behind transition amplitude simulation.
% vertex amplitude of quantum spacetime in ooguris model. ooguri defines a topological invariant four-manifold

        %In 3-dimensional space states single vertex and two vertex cases can find. On the other hand, for the large spin foam amplitudes 
        %3-dimensional space states that correspond to the 3-dimensional boundaries of classical 4-dimensional simplicial complexes which can be represent with tetrahedrons.

        %The spin foam amplitudes are the essential part of the 4-dimensional quantum spacetime region. The vertex is the 3-dimensional 
        %boundary space states of space-timespace time atoms with five quantum tetrahedrons.
...
\section{Quantum Simulation}
        %To simulate the spin-foam amplitudes of the vertex state, %of a spin-network graph, 
...
\subsection{Circuit}
        ...
        \begin{figure}[h!]
                ...
                %\includegraphics[scale=.3,natwidth=600,natheight=600]{images/qtet_state_prep.png}
                \includegraphics[scale=.3]{images/qtet_state_prep.png}
                \caption{$U_{0-3}$}
                \end{subfigure}
                \begin{subfigure}[b]{.3\textwidth}
                %\includegraphics[scale=.3,natwidth=600,natheight=600]{images/inv_bell_state.png}
...
\subsection{Topology}
        %We will also pay attention to the topology of the quantum circuit. %the Ooguri model and topological invariance, CITATION NEEDED
        ...
        are uniform, this transition amplitude is a dependent variable of, mostly, the topology of the quantum circuit. In this case, the final wavefunction  %We will provide an analytical proof if 
...
\subsection{Dipole Spin Network}
        ...
        dipole cosmology \cite{homogeneity-lqc}. We can organize expected values into two topological groups: one chain and two chains. Expected values are shown in the table below.         %Two different configurations of the dipole spin-network were performed. This is the gluing of two quantum tetrahedra to form a maximally entangled spin network.
        %There was a hardware error with the processor that we had access to, so only  one experiment has useable results. For the network $04,17,25,36$ we get the following. 
        \begin{table}[h]
        \begin{centering}
                \begin{tabular}{|l|l|}
                        \hline
        ...
        \begin{figure}[h] \label{dipole-topology}
                ...
                %\includegraphics[scale=.2,natwidth=600,natheight=600]{images/dipole_disjoint_circuit.png}
                \includegraphics[scale=.2]{images/dipole_disjoint_circuit.png}
                \caption{$B^\dagger_{04,15,26,37}U$ \cite{cirq}}
                \end{subfigure}
                \begin{subfigure}[b]{.3\textwidth}
                %\includegraphics[scale=.1,natwidth=600,natheight=600]{images/dipole_disjoint.jpg}
                \includegraphics[scale=.1]{images/dipole_disjoint.jpg}
                \caption{Double topology, $04,15,26,37$}
                \end{subfigure}
                \begin{subfigure}[b]{.2\textwidth}
                %\includegraphics[scale=.1,natwidth=600,natheight=600]{images/dipole_union.jpg}
...
\subsection{4-Simplex Spin Network}
        ...
        \begin{figure}[h] \label{simplex-topology}
                ...
                \end{subfigure}
                \begin{subfigure}[b]{.2\textwidth}
                %\includegraphics[scale=.05,natwidth=600,natheight=600]{images/simplex-donuts.jpg}
                \includegraphics[scale=.05]{images/simplex-donuts.jpg}
                \caption{Double topology}
                \end{subfigure}
                \begin{subfigure}[b]{.3\textwidth}
                %\includegraphics[scale=.05,natwidth=600,natheight=600]{images/simplex-beans.jpg}
...
\subsection{Submission One}        
        % DISCUSS qubits errors
        % Results for one experiment of dipole, post selection
        % fixes for final state
